...
vector<glm::vec4> GetFrustumPlanes(const glm::mat4& transform) 
{
	...
	{
		parts[i] = glm::vec4(transform[0][i],
		                     transform[1][i],
		                     transform[2][i],
		                     transform[3][i]);
	}
...
bool BoundingSphere::IsTouchingFrustumPlanes(const glm::mat4& transform, glm::vec3 positionOfBoundingSphere) 
{
